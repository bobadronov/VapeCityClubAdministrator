...
internal class UkrainianPhoneVisualTransformation : VisualTransformation {
    ...
    override fun filter(text: AnnotatedString): TransformedText {
        ...
        val inputText = text.text.take(10)
        val digits = inputText.filter { it.isDigit() }

        // Обмежуємо до 10 цифр для формату +38 (XXX) XXX XX XX
        val trimmed = digits.take(10)

        // Обробка порожнього вводу
        if (trimmed.isEmpty()) {
            return TransformedText(
                androidx.compose.ui.text.AnnotatedString("+38 "),
        ...
        if (trimmed.length <= 3) {
            ...
            return TransformedText(
                androidx.compose.ui.text.AnnotatedString(builder.toString()),
                object : OffsetMapping {
